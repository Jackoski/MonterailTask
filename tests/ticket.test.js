...
test('should create new ticekts', async () => {
    const response = await request(app).post(`/event/ticket/add/${eventOne._id}`).send({
        quantity: 100,
        price: 50,
        event: eventOne._id
    }).expect(201);
...
test('should get all tickets', async () => {
    const response = await request(app).get('/event/ticket').send().expect(200);
    const tickets = await Ticket.find();
    expect(tickets).not.toBeNull();
});

test('should get specific tickets for specific event', async () => {
    const response = await request(app).get(`/event/ticket/${ticketOne._id}`).send()
        .expect(200);
    const ticket = await Ticket.findById(response.body._id);
...
test('should not get a specific tickets for specific event', async () => {
    const response = await request(app).get(`/event/ticket/5cb0658d2a52e240d4f4df56`).send()
        .expect(404);
    const ticket = await Ticket.findById(response.body._id);
    expect(ticket).toBeNull();
});
...
test('should edit tickets', async () => {
    const response = await request(app).patch(`/event/ticket/edit/${ticketOne._id}`).send({
        quantity: 500,
        price: 123
    }).expect(200);
    const ticket = await Ticket.findById(response.body._id);
...
test('should buy a ticket', async () => {
    const response = await request(app).patch(`/event/ticket/buy/${ticketOne._id}?token=ok`).send({
        quantity: 2
    }).expect(200);
...
test('should be card_error during buying a ticket', async () => {
    const response = await request(app).patch(`/event/ticket/buy/${ticketOne._id}?token=card_error`).send({
        quantity: 2
    }).expect(400);
    expect(response.body.error).not.toBeNull();
});
test('should be card_error during buying a ticket', async () => {
    const response = await request(app).patch(`/event/ticket/buy/${ticketOne._id}?token=payment_error`).send({
        quantity: 2
    }).expect(400);
    expect(response.body.error).not.toBeNull();
});